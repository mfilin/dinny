...
{
    public partial class frmDishList : frmChild
    {
        public Dish Selected
        {
            get
            {
                if (this.grid.CurrentRow != null)
                {
                    Dish row = this.grid.CurrentRow.DataBoundItem as Dish;
                    return row;
                }
                return null;
            }
        ...
        {
            IList<Dish> source = new DishManager().GetAll();
        ...
        private void Edit(Dish lpEntity)
        {
        ...
        private void extToolStrip_OnDeleteClick(object sender, EventArgs e)
        {
            if (null != this.Selected)
            {
                try
                {
                    new DishManager().CheckRelations(this.Selected);
                    new DishManager().Delete(this.Selected);
                    this.RefreshGrid();
                }
        ...
        private void extToolStrip_OnPrintClick(object sender, EventArgs e)
        {
            if (null != this.Selected)
            {
                dinny.Reports.rptViewer.ExecTCard(this.Selected);
            }
        }
