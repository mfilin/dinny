...
{
    public partial class frmPersonList : frmChild
    {
        ...
        public Person Selected
        {
            get
            {
                return (null == this.dataTree.SelectedObject) ? _selected :
                    this.dataTree.SelectedObject as Person;
            }
            set
            {
                this._selected = this.dataTree.SelectedObject as Person;
            }
        }
        ...
        private void frmPersonList_Load(object sender, EventArgs e)
        {
            ...
            this.dataTree.CanExpandGetter = x => (x as Person).ChildCount() > 0;
            this.dataTree.ChildrenGetter = x => (x as Person).Children;
            // картинки для узлов
            this.olvColumnID.ImageGetter = x => (x as Person).parent == null && (x as Person).parentid == null ? 0 : -1; 
        }

        private void RefreshGrid()
        {
            IList<Person> treesource = new PersonManager().GetByCriteria(NHibernate.Criterion.Restrictions.IsNull("parentid")
            ...
            //IList<TypePerson> source = new TypePersonManager().GetPage(this.loCurrentPage, this.loPageDef, this.gridFilter);
            //if (null != source)
            //{
        ...
        private void Edit(Person lpEntity)
        {
        ...
        private void extToolStrip_OnAddClick(object sender, EventArgs e)
        {
            ...
            if (null != this.Selected)
            {
                child = new Person("", this.Selected);
            }
            else
                child = new Person("");
        ...
        private void extToolStrip_OnDeleteClick(object sender, EventArgs e)
        {
            if (null != this.Selected)
            {
                ...
                new PersonManager().Delete(this.Selected);
                this.RefreshGrid();
            }
        }
        ...
        private void dataTree_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            this.Selected = this.dataTree.SelectedObject as Person;
